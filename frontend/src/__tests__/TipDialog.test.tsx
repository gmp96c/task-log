...
test('loads and displays tips in order of most used', async () => {
    const element = render(
        <MockedProvider mocks={mocks}>
            <TipDialog tipOpen setTipOpen={() => {}} task={testTask} selected={[testTask.tips[0], testTask.tips[2]]} />
        </MockedProvider>,
    );
    ...
    const elementsInOrder = Array.from(element.baseElement.children);
    console.log(elementsInOrder);
    // Makes sure tip array is sorted high to low
    const tipArr = testTask.tips.sort((a, b) => (a._pinnedByMeta.count < b._pinnedByMeta.count ? 1 : -1));
        ...
        <MockedProvider mocks={mocks}>
            <UserContextWrapper value={testUser}>
                <TipDialog
                    ...
                    setTipOpen={() => {}}
                    task={testTask}
                    selected={[testTask.tips[0], testTask.tips[2]]}
                />
            </UserContextWrapper>
        </MockedProvider>,
    );
    // get tips
    const getTips = Array.from(element.container.querySelectorAll('.tip'));

    // get tip input
    const input = screen.getByPlaceholderText('Add New Tip');
    if (input === null) {
